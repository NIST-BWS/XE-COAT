...
public abstract class Tests extends TemplateServicesTests {
	...
	public void settingAndGettingATemplateRoundtripsProperly()
		...
		ts.createService("service0");
		ts.setTemplate("service0", Examples.TEMPLATE.getData(),
				Examples.TEMPLATE.getContentType("plain"));
		String roundTrip = StringSourceConverters.fromResponse(
				ts.getTemplate("service0")).getString();
		assertEquals(Examples.TEMPLATE.getString(), roundTrip);
	}

	@Test
	public void settingAndGettingATemplateSetsRelHeaderProperly()
			throws UnsupportedEncodingException {
		ts.createService("service0");
		ts.setTemplate("service0", Examples.TEMPLATE.getData(),
				Examples.TEMPLATE.getContentType("plain"));
		assertEquals(
				ts.getTemplate("service0").getHeaderString(
						Constants.HttpHeader.REL), Constants.Rel.TEMPLATE);
	}
	...
	public void settingATemplateProvidesAResourceLink() {
		...
		ts.setTemplate(serviceName, Examples.TEMPLATE.getData(),
				Examples.TEMPLATE.getContentType("plain"));
		List<Link> templates = ts.getServiceResources(serviceName)
				.getTemplateLinks();

		assertEquals(1, templates.size());
		assertEquals(Constants.TEMPLATE_RESOURCE_NAME, templates.get(0)
				.getName());
		assertEquals(Constants.Rel.TEMPLATE, templates.get(0).getRel());
		assertEquals(BuildUri.getTemplateUri(serverRootUri, serviceName),
				templates.get(0).getUri());
	...
	public void deletingATemplateWorks() {
		...
		int template_count_before_delete = ts.getServiceResources(serviceName)
				.getTemplateLinks().size();
		ts.deleteTemplate(serviceName);
		int template_count_after_delete = ts.getServiceResources(serviceName)
				.getTemplateLinks().size();
	...
	public void settingATemplateWithAnInvalidCharsetIsForbidden() {
		...
		ts.createService(serviceName);
		ts.setTemplate(serviceName, Examples.TEMPLATE.getData(),
				"text/plain; charset=badcharsetname");
	}
	...
	public void settingATemplateWithAnEmptyCharsetTriggersCharsetDetection() {
		...
		StringSource rt = StringSourceConverters.fromResponse(ts
				.getTemplate(serviceName));
		assertEquals(Examples.TEMPLATE.getCharset(), rt.getCharset());
	}

	@Test
	public void getTemplateShouldReturnCorrectHeaders() {
		...
		javax.ws.rs.core.Response r = ts.getTemplate(serviceName);
		assertEquals(serviceName, r.getHeaderString(HttpHeader.SERVICE_NAME));
		assertEquals("template", r.getHeaderString(HttpHeader.RESOURCE_NAME));
		assertEquals("template", r.getHeaderString(HttpHeader.REL));
