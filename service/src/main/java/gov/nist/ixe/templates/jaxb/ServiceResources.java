...
public class ServiceResources extends ResourceList {
	...
	public ServiceResources(IStorageProvider storage, String rootUri, String serviceName) {
		...
		if (hasTemplate && hasPrimarySchema && hasDefaultConfig) {
			...
			for (String configName : storage.getConfigNames(serviceName)) {
				resources.add(Link.NamedProcess(storage, rootUri, serviceName, configName));
			}
			
		}
		
		for (String schemaName : storage.getSchemaNames(serviceName)) {
			resources.add(Link.Schema(storage, rootUri, serviceName, schemaName));
		}

		for (String configName : storage.getConfigNames(serviceName)) {
			resources.add(Link.Config(storage, rootUri, serviceName, configName));
		}
	...
	public boolean hasConfig(String configName) {
		...
		for (Link link : links) {
			if (link.getName().equals(configName)) return true;
		}
		return false;
	}
